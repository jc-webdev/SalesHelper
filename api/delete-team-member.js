...
export default async function handler(request, response) {
    ...
    if (!memberId) {
        return json(response, 400, { error: 'memberId is required' });
    }
    ...
    if (memberId === userData.user.id) {
        return json(response, 400, { error: 'Nie możesz usunąć własnego konta' });
    }

    const { error: deleteError } = await supabase.auth.admin.deleteUser(memberId);
    if (deleteError) {
        return json(response, 400, { error: deleteError.message });
    }

    return json(response, 200, { id: memberId });
}
